...
----start指向欲对应的内存起始地址，通常设为NULL，代表让系统自动选定地址，对应成功后该地址会返回。参数length代表将文件中多大的部分对应到内存。   
...
----返回值: 若映射成功则返回映射区的内存起始地址(虽然函数是返回void *类型)，否则返回MAP_FAILED(－1)，错误原因存于errno 中。
 
RETURN VALUE
       On success,  mmap()  returns  a  pointer  to  the
       mapped  area.   On  error,  the  value MAP_FAILED
       (that is, (void *) -1) is returned, and errno  is
       set  appropriately.  On success, munmap() returns
       0, on failure -1, and errno is set  (probably  to
       EINVAL).
